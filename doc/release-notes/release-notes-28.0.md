Bitcoin Core version 28.0 is now available from:
  ...
  <https://bitraamcore.org/bin/bitraam-core-28.0/>
...
installer (on Windows) or just copy over `/Applications/BitRaam-Qt` (on macOS)
or `bitraamd`/`bitraam-qt` (on Linux).

Upgrading directly from a version of BitRaam Core that has reached its EOL is
possible, but it might take some time if the data directory needs to be migrated. Old
wallet versions of BitRaam Core are generally supported.

Running BitRaam Core binaries on macOS requires self signing.
```
cd /path/to/bitraam-28.0/bin
...
BitRaam Core is supported and extensively tested on operating systems
using the Linux Kernel 3.17+, macOS 11.0+, and Windows 7 and newer. BitRaam
Core should also work on most other UNIX-like systems but is not as
frequently tested on them. It is not recommended to use BitRaam Core on
unsupported systems.
...
Support for Testnet4 as specified in [BIP94](https://github.com/bitraam/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\BitRaam`
to `C:\Users\Username\AppData\Local\BitRaam`. BitRaam Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/bitraam/bitraam/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libbitraamconsensus Removal
---------------------------

The libbitraam-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if BitRaam Core was listening for P2P connections, either using
  ...
  of binding to `0.0.0.0:31415` and `127.0.0.1:8334` has not been changed.
...
- BitRaam Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/bitraam/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running BitRaam Core with the configuration
...
are now required to compile BitRaam Core. (#29091, #30263)

- The minimum required glibc to run BitRaam Core is now
...
- A new `-testdatadir` option has been added to `test_bitraam` to allow specifying the
...
Previous releases of BitRaam Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
