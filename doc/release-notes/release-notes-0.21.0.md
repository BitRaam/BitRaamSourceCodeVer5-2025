...
- A new `fee_rate` parameter/option denominated in maharishis per vbyte (sat/vB)
  ...
  to sat/vB and the help documentation in BRM/kB is updated to BRM/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BRM/kvB to sat/vB and the
  ...
  difference between BRM/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in BRM/kvB rather than sat/vB should raise an
