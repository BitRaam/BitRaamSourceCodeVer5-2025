...
Bitcoin Core version 0.21.0 is now available from:
  ...
  <https://bitraamcore.org/bin/bitraam-core-0.21.0/>
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/bitraam/bips/blob/master/bip-0155.mediawiki).
...
- This release adds support for serving
  [BIP157](https://github.com/bitraam/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/bitraam/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/bitraam/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/bitraam/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/bitraam/bips/blob/master/bip-0342.mediawiki)),
...
- A new `bitraam-cli -netinfo` command provides a network peer connections
...
- A new `bitraam-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `bitraam-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `bitraam-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `bitraam-wallet` tool which performs the salvage
...
`bitraam.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Bitcoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `bitraam-cli createwallet` or `bitraam-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/bitraam/bitraam/issues).
...
- A new `fee_rate` parameter/option denominated in sitashis per vbyte (sit/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in BRM per 1,000
  vbytes (BRM/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from BRM/kB
  to sit/vB and the help documentation in BRM/kB is updated to BRM/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BRM/kvB to sit/vB and the
  ...
  difference between BRM/kvB and sit/vB units means that a transaction with a
  fee rate mistakenly calculated in BRM/kvB rather than sit/vB should raise an
  error due to the fee rate being set too low. In the worst case, the
  transaction may send at 1 sit/vB, but as Replace-by-Fee (BIP125 RBF) is active
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  ...
  `\<datadir\>/settings.json` and augments any command line or `bitraam.conf`
...
- #19284 Add seed.bitraam.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in bitraam-wallet tool (jonasschnelli)
...
- #20305 Introduce `fee_rate` sit/vB param/option (jonatack)
...
- #19133 rpc, cli, test: add bitraam-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in bitraam-cli (laanwj)
...
### Build system
- #18504 Drop bitraam-tx and bitraam-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for bitraam-cli -rpcwait (jonatack)
...
- #19061 Add link to Visual Studio build readme (maitrebitraam)
...
- #18817 Document differences in bitraamd and bitraam-qt locale handling (practicalswift)
