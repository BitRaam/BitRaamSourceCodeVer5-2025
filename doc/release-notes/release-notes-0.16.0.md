Bitcoin Core version 0.16.0 is now available from:
  ...
  <https://bitraamcore.org/bin/bitraam-core-0.16.0/>
...
bitraam data directory. The behavior is now:
...
GUI changes
-----------
- Uses of "µBRM" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- bitraam-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- bitraam-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 131416.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest bitraam.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.bitraam.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for bitraam-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to bitraam-cli to allow RPC password to be read from standard input (jharvell)
...
- #10825 `28485c7` Set regtest JSON-RPC port to 18443 to avoid conflict with testnet 131416 (fametrano)
...
- #12035 `eeb6d52` Change µBRM to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_bitraam_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` bitraam_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.bitraam doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for bitraam-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in bitraam-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop bitraamd (OmeGak)
...
- #10529 `331352f` Improve bitraamd systemd service file (Flowdalic)
