Bitcoin Core version 0.19.0.1 is now available from:
  ...
  <https://bitraamcore.org/bin/bitraam-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/bitraam/bitraam/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bitraamd -help` output for these two options for
...
- The `testnet` field in `bitraam-cli -getinfo` has been renamed to
...
- If a Tor hidden service is being used, Bitcoin Core will be bound to
  the standard port 31415 even if a different port is configured for
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  BRM), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in BRM per 1,000
  vbytes, not sitashi per vbyte. (#16257)
...
- #15639 bitraam-wallet tool: Drop libbitraam_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BRM/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in bitraamd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "bitraam-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of bitraamd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from bitraam_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_bitraam to auto generated project files (sipsorcery)
...
- #15773 Add BitRaamTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_bitraam output (practicalswift)
...
- #15896 feature_filelock, interface_bitraam_cli: Use PACKAGE_NAME in messages rather than hardcoding Bitcoin Core (luke-jr)
...
- #16277 Suppress output in test_bitraam for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as BRM/kB (jonatack)
...
- #16489 log: harmonize bitraamd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (bitraamVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example bitraam.conf (fanquake)
...
- #16080 build/doc: update bitraam_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in bitraam.conf (fanquake)
...
- #16621 add default bitraam.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bitraam-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bitraam.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/bitraamd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/bitraam/bitraam/).
