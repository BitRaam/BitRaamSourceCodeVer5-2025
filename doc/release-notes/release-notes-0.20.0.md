...
Bitcoin Core version 0.20.0 is now available from:
  ...
  <https://bitraamcore.org/bin/bitraam-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/bitraam/bitraam/issues/16599),
  [PR #16702](https://github.com/bitraam/bitraam/pull/16702), and the
  `bitraamd help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://bitraamcore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/bitraam-core/HWI) for
...
- Bitcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.bitraamcore.org.  (#17596)
...
- The `bitraam-cli` utility used with the `-getinfo` parameter now
...
- The `bitraam-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=31415` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `bitraamstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitraam-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_bitraam` binary (hebasto)
...
- #18702 Fix ASLR for bitraam-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate bitraam schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitraam.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitraam` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitraam-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone bitraam-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken bitraam-cli examples (andrewtoth)
...
- #17503 Remove bitness from bitraam-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.bitraamcore.org (laanwj)
...
- #17648 Rename wallet-tool references to bitraam-wallet (hel-o)
...
- #18341 Replace remaining literal BRM with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitraamd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitraam-cli -getinfo (malevolent)
