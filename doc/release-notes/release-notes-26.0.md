...
Bitcoin Core version 26.0 is now available from:
  ...
  <https://bitraamcore.org/bin/bitraam-core-26.0/>
...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/bitraam/bips/blob/master/bip-0324.mediawiki) was added.
...
- [Miniscript](https://bitraam.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://bitraam.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `bitraamd` and `bitraam-qt` will now raise an error on startup
 if a datadir that is being used contains a bitraam.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a bitraam.conf file. The error message is just a diagnostic intended
 ...
 the bitraam.conf contained in it. (#27302)
...
- A new `bitraamconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `bitraamconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
```sh
src/bitraam-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/bitraam-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://bitraam.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
...
- Bash completion files have been renamed from `bitraam*.bash-completion` to
  `bitraam*.bash`. This means completions can be automatically loaded on demand
