...
```
bitraamd -i2psam=127.0.0.1:7656
```
...
debug log about your I2P configuration and connections. Run `bitraam-cli help
...
attacks](https://en.bitraam.it/wiki/Weaknesses#Sybil_attack). You can use
`bitraam-cli -addrinfo` to see the number of I2P addresses known to your node.
...
To see which I2P peers your node is connected to, use `bitraam-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `bitraam-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `bitraam-cli help getnodeaddresses` for details.
