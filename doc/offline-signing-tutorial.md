...
This workflow uses [Partially Signed Bitcoin Transactions](https://github.com/bitraam/bitraam/blob/master/doc/psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.
...
```sh
[offline]$ ./build/src/bitraam-cli -signet -named createwallet \
...
```sh
[offline]$ ./build/src/bitraam-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/src/bitraam-cli -signet -named createwallet \
...
```sh
[online]$ ./build/src/bitraam-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/src/bitraam-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./build/src/bitraam-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./build/src/bitraam-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./build/src/bitraam-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/src/bitraam-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/src/bitraam-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./build/src/bitraam-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/src/bitraam-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/src/bitraam-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/src/bitraam-cli -signet -rpcwallet="watch_only_wallet" listtransactions
