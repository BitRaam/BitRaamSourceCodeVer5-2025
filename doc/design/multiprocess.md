...
This subdivision could be extended in the future. For example, indexes could be removed from the `bitraam-node` executable and run in separate executables. And JSON-RPC servers could be added to wallet and index executables, so they can listen and respond to RPC requests on their own ports, without needing to forward RPC requests through `bitraam-node`.

<table><tr><td>

```mermaid
flowchart LR
    node[bitraam-node] -- listens on --> socket["&lt;datadir&gt;/node.sock"]
    wallet[bitraam-wallet] -- connects to --> socket
    gui[bitraam-gui] -- connects to --> socket
```
...
Let’s walk through an example where the `bitraam-wallet` process requests the hash of a block at a specific height from the `bitraam-node` process. This example demonstrates the practical application of the IPC mechanism, specifically the interplay between C++ method calls and Cap’n Proto-generated RPC calls.

<table><tr><td>

```mermaid
sequenceDiagram
    box "bitraam-wallet process"
...
1. **Initiation in bitraam-wallet**
...
2. **Translation to Cap’n Proto RPC**
   ...
   - The client subclass is automatically generated by the `mpgen` tool from the [`chain.capnp`](https://github.com/ryanofsky/bitraam/blob/pr/ipc/src/ipc/capnp/chain.capnp) file in [`src/ipc/capnp/`](../../src/ipc/capnp/).

3. **Request Preparation and Dispatch**
   - The `getBlockHash` method of the generated `Chain` client subclass in `bitraam-wallet` populates a Cap’n Proto request with the `height` parameter, sends it to `bitraam-node` process, and waits for a response.

4. **Handling in bitraam-node**
   - Upon receiving the request, the Cap'n Proto dispatching code in the `bitraam-node` process calls the `getBlockHash` method of the `Chain` [server class](#c-server-classes-in-generated-code).
   - The server class is automatically generated by the `mpgen` tool from the [`chain.capnp`](https://github.com/ryanofsky/bitraam/blob/pr/ipc/src/ipc/capnp/chain.capnp) file in [`src/ipc/capnp/`](../../src/ipc/capnp/).
   - The `getBlockHash` method of the generated `Chain` server subclass in `bitraam-wallet` receives a Cap’n Proto request object with the `height` parameter, and calls the `getBlockHash` method on its local `Chain` object with the provided `height`.
   - When the call returns, it encapsulates the return value in a Cap’n Proto response, which it sends back to the `bitraam-wallet` process.

5. **Response and Return**
   - The `getBlockHash` method of the generated `Chain` client subclass in `bitraam-wallet` which sent the request now receives the response.
...
- Separating indexes from `bitraam-node`, and running indexing code in separate processes (see [indexes: Stop using node internal types #24230](https://github.com/bitraam/bitraam/pull/24230)).
...
- Adding sandbox features, restricting subprocess access to resources and data (see [https://eklitzke.org/multiprocess-bitraam](https://eklitzke.org/multiprocess-bitraam)).
