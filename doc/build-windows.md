...
Below are some notes on how to build BitRaam Core for Windows.

The options known to work for building BitRaam Core on Windows are:
...
Note that for WSL the BitRaam Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitraam, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitraam`, for example:

    cmake --install build --prefix /mnt/c/workspace/bitraam
