# CJDNS support in BitRaam Core

It is possible to run BitRaam Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitRaam Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitRaam network and individual nodes.
...
before it will work with your BitRaam Core node. This is described in steps
...
## Run BitRaam Core with CJDNS

Once you are connected to the CJDNS network, the following BitRaam Core
...
When enabled, this option tells BitRaam Core that it is running in an
...
IPv6 local network. This helps BitRaam Core perform better address management:
...
CJDNS support was added to BitRaam Core in version 23.0 and there may be fewer
...
## CJDNS-related information in BitRaam Core

There are several ways to see your CJDNS address in BitRaam Core:
