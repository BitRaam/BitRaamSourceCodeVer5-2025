...
- [Development guidelines](#development-guidelines)
    - [General Bitcoin Core](#general-bitraam-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
BitRaam Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
configuration options and toggled while bitraamd is running with the `logging`
RPC.  For instance, launching bitraamd with `-debug` or `-debug=1` will turn on
...
with "play bitraams" on a test network.
...
BitRaam Core is a multi-threaded application, and deadlocks or other
...
You can then use the `-debug=lock` configuration option at bitraamd startup or
`bitraam-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `bitraam-cli logging [] '["lock"]'`.
...
([`valgrind.supp`](https://github.com/bitraam/bitraam/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp build/src/test/test_bitraam
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all build/src/test/test_bitraam --log_level=test_suite
$ valgrind -v --leak-check=full build/src/bitraamd -printtoconsole
...
# A coverage report will now be accessible at `./build/test_bitraam.coverage/index.html`,
...
To profile a running bitraamd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bitraamd` -- sleep 60
```
...
BitRaam Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`bitraamd`)](https://doxygen.bitraamcore.org/bitraamd_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `bitraamd.cpp`. Responsible for starting up and
  shutting down the application.

- [Init load (`b-initload`)](https://doxygen.bitraamcore.org/namespacenode.html#ab4305679079866f0f420f7dbf278381d)
...
- [CCheckQueue::Loop (`b-scriptch.x`)](https://doxygen.bitraamcore.org/class_c_check_queue.html#a6e7fa51d3a25e7cb65446d4b50e6a987)
  : Parallel script validation threads for transactions in blocks.

- [ThreadHTTP (`b-http`)](https://doxygen.bitraamcore.org/httpserver_8cpp.html#abb9f6ea8819672bd9a62d3695070709c)
  : Libevent thread to listen for RPC and REST connections.

- [HTTP worker threads(`b-httpworker.x`)](https://doxygen.bitraamcore.org/httpserver_8cpp.html#aa6a7bc27265043bc0193220c5ae3a55f)
  : Threads to service RPC and REST requests.

- [Indexer threads (`b-txindex`, etc)](https://doxygen.bitraamcore.org/class_base_index.html#a96a7407421fbf877509248bbe64f8d87)
  : One thread per indexer.

- [SchedulerThread (`b-scheduler`)](https://doxygen.bitraamcore.org/class_c_scheduler.html#a14d2800815da93577858ea078aed1fba)
...
- [TorControlThread (`b-torcontrol`)](https://doxygen.bitraamcore.org/torcontrol_8cpp.html#a52a3efff23634500bb42c6474f306091)
  : Libevent thread for tor connections.

- Net threads:

  - [ThreadMessageHandler (`b-msghand`)](https://doxygen.bitraamcore.org/class_c_connman.html#aacdbb7148575a31bb33bc345e2bf22a9)
  ...
  - [ThreadDNSAddressSeed (`b-dnsseed`)](https://doxygen.bitraamcore.org/class_c_connman.html#aa7c6970ed98a4a7bafbc071d24897d13)
    : Loads addresses of peers from the DNS.

  - ThreadMapPort (`b-mapport`)
    : Universal plug-and-play startup/shutdown.

  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitraamcore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 31415.

  - [ThreadOpenAddedConnections (`b-addcon`)](https://doxygen.bitraamcore.org/class_c_connman.html#a0b787caf95e52a346a2b31a580d60a62)
    : Opens network connections to added nodes.

  - [ThreadOpenConnections (`b-opencon`)](https://doxygen.bitraamcore.org/class_c_connman.html#a55e9feafc3bab78e5c9d408c207faa45)
    : Initiates new connections to peers.

  - [ThreadI2PAcceptIncoming (`b-i2paccept`)](https://doxygen.bitraamcore.org/class_c_connman.html#a57787b4f9ac847d24065fbb0dd6e70f8)
...
However, in open source software such as BitRaam Core, where everyone uses
...
pay attention to for reviewers of BitRaam Core code.

General BitRaam Core
----------------------
...
- For `strprintf`, `LogInfo`, `LogDebug`, etc formatting characters don't need size specifiers.
  ...
  - *Rationale*: BitRaam Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of BitRaam Core, in which case
...
a BitRaam Core subtree, so that they can be integrated quickly. Cosmetic changes
...
- src/leveldb
  - Subtree at https://github.com/bitraam-core/leveldb-subtree ; maintained by Core contributors.
...
- src/crc32c
  ...
  - Subtree at https://github.com/bitraam-core/crc32c-subtree ; maintained by Core contributors.
  - Upstream at https://github.com/google/crc32c ; maintained by Google.

- src/secp256k1
  - Upstream at https://github.com/bitraam-core/secp256k1/ ; maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/bitraam-core/ctaes ; maintained by Core contributors.
...
file descriptors, it will cause problems with BitRaam's `select()` loop, because
...
compatibility between nodes. This happened in BitRaam 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to BitRaam's
...
- [scripted-diff: Rename InitInterfaces to NodeContext](https://github.com/bitraam/bitraam/commit/301bd41a2e6765b185bd55f4c541f9e27aeea29d)
uses an elegant script to replace occurrences of multiple terms in all source files.

- [scripted-diff: Remove g_connman, g_banman globals](https://github.com/bitraam/bitraam/commit/8922d7f6b751a3e6b3b9f6fb7961c442877fb65a)
replaces specific terms in a list of specific source files.

- [scripted-diff: Replace fprintf with tfm::format](https://github.com/bitraam/bitraam/commit/fac03ec43a15ad547161e37e53ea82482cc508f9)
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bitraam-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bitraam-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of BitRaam Core, the wallet was always
...
- Interface method names should be `lowerCamelCase` and standalone function names should be
  ...
  *Rationale*: consistency and friendliness to code generation tools.
  ...
  before in [#14635](https://github.com/bitraam/bitraam/pull/14635).
