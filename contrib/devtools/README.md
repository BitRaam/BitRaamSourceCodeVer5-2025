...
Generates a bitraam.conf file in `share/examples/` by parsing the output from `bitraamd --help`. This script is run during the
release process to include a bitraam.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/build contrib/devtools/gen-bitraam-conf.sh
```
...
If there are any errors the return value will be 1 and output like this will be printed:
    ...
    .../64/test_bitraam: symbol memcpy from unsupported version GLIBC_2.14
    .../64/test_bitraam: symbol __fdelt_chk from unsupported version GLIBC_2.15
    .../64/test_bitraam: symbol std::out_of_range::~out_of_range() from unsupported version GLIBCXX_3.4.15
    .../64/test_bitraam: symbol _ZNSt8__detail15_List_nod from unsupported version GLIBCXX_3.4.15
