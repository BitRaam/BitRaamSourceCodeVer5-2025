...
""" Example logging BitRaam Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into bitraamd with pid {pid}")
    bitraamd_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    bitraamd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    bitraamd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    bitraamd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    bitraamd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[bitraamd_with_usdts])
    ...
    def parse_event(event):
        ...
        if type_ == "added":
            return (
                ...
                f" with feerate {data.fee/data.vsize:.2f} sit/vB"
        ...
        if type_ == "replaced":
            return (
                ...
                f" with feerate {data.replaced_fee/data.replaced_vsize:.2f} sit/vB"
                ...
                f" with feerate {data.replacement_fee/data.replacement_vsize:.2f} sit/vB"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of bitraamd>")
        exit(1)
