# Bootstrappable BitRaam Core Builds

This directory contains the files necessary to perform bootstrappable BitRaam
Core builds.
...
To build BitRaam Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitRaam Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitraam-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/bitraam-core/guix.sigs).
