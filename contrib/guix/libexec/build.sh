...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/bitraam-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our BitRaam Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built BitRaam Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    case "$HOST" in
        *darwin*)
            ...
            mv build/dist/BitRaam-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
    ...
    (
        ...
        # copy over the example bitraam.conf file. if contrib/devtools/gen-bitraam-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/bitraam.conf" "${DISTNAME}/"
