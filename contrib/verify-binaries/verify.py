...
"""Script for verifying BitRaam Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from bitraamcore.org and bitraam.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/bitraam-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://bitraamcore.org"
HOST2 = "https://bitraam.org"
VERSIONPREFIX = "bitraam-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the BitRaam community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"bitraam_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by bitraamcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the bitraam release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes bitraam.org lags behind bitraamcore.org.)')
    )
