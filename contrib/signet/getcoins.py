...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitraam-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitraam-cli', help='bitraam-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitRaam address to which the faucet should send')
...
parser.add_argument('bitraam_cli_args', nargs='*', help='Arguments to pass on to bitraam-cli (default: -signet)')

args = parser.parse_args()

if args.bitraam_cli_args == []:
    args.bitraam_cli_args = ['-signet']


def bitraam_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitraam_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitraam_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitraam_cli(['getnewaddress', 'faucet', 'bech32'])
