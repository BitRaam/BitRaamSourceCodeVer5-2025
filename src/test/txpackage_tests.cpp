...
// A fee amount that is above 1sit/vB but below 5sit/vB for most transactions created within these
// unit tests.
...
BOOST_AUTO_TEST_CASE(package_cpfp_tests)
{
    ...
    // The mempool minimum feerate is 5sit/vB, but this package just pays 800 sitashis total.
    ...
    // Now that the child's fees have "increased" by 1 BRM, the cheap package should succeed.
    {
    ...
    // Parent pays 1 BRM and child pays none. The parent should be accepted without the child.
    {
        BOOST_CHECK_EQUAL(m_node.mempool->size(), expected_pool_size);
