...
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // instead of strings, because bitraam 22.x will not try to read these.
...
//! Convert enabled/size values to bitraam -prune setting.
...
//! Get pruning enabled value to show in GUI from bitraam -prune setting.
...
//! Get pruning size value to show in GUI from bitraam -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (unit.canConvert<BitcoinUnit>()) {
        ...
        m_display_bitraam_unit = BitcoinUnit::BTC;
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitraam.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitraam_unit);
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/bitraam/bitraam/pull/8273
    ...
    // (https://github.com/bitraam-core/gui/issues/567).
