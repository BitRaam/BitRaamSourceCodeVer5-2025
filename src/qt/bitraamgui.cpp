...
#include <bitraam-build-config.h> // IWYU pragma: keep

#include <qt/bitraamgui.h>
...
const std::string BitRaamGUI::DEFAULT_UIPLATFORM =
...
BitRaamGUI::BitRaamGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &BitRaamGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BitRaamGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BitRaamGUI::showModalOverlay);
...
void BitRaamGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BitRaam address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitraam: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &BitRaamGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BitRaamGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitRaamGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BitRaam addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BitRaam addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed BitRaam Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed BitRaam Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a bitraam: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitRaam command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &BitRaamGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BitRaamGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BitRaamGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BitRaamGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BitRaamGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BitRaamGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BitRaamGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BitRaamGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &BitRaamGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &BitRaamGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &BitRaamGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &BitRaamGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BitRaamGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BitRaamGUI::showDebugWindow);
}

void BitRaamGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void BitRaamGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BitRaamGUI::setCurrentWalletBySelectorIndex);
...
void BitRaamGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BitRaamGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BitRaamGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BitRaamGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BitRaamGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BitRaamGUI::enableHistoryAction(bool privacy)
{
...
void BitRaamGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BitRaamGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BitRaamGUI::removeWallet);
...
WalletController* BitRaamGUI::getWalletController()
{
    return m_wallet_controller;
}

void BitRaamGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BitRaamGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BitRaamGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BitRaamGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BitRaamGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BitRaamGUI::incomingTransaction);
    connect(this, &BitRaamGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void BitRaamGUI::removeWallet(WalletModel* walletModel)
{
...
void BitRaamGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void BitRaamGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BitRaamGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitRaamGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitRaamGUI::createTrayIcon()
{
...
void BitRaamGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BitRaamGUI::toggleHidden);
...
void BitRaamGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BitRaamGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitRaamGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BitRaamGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BitRaamGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BitRaamGUI::openClicked()
{
...
void BitRaamGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitRaamGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitRaamGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitRaamGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitRaamGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitRaamGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BitRaamGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BitRaamGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to BitRaam network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BitRaamGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitRaamGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BitRaamGUI::updateHeadersSyncProgressLabel()
{
...
void BitRaamGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void BitRaamGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BitRaamGUI::quitRequested);
...
void BitRaamGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BitRaamGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &BitRaamGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void BitRaamGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BitRaamGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BitRaamGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BitRaamGUI::show);
                e->ignore();
            }
        }
    }
...
void BitRaamGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitRaamGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitRaamGUI::incomingTransaction(const QString& date, BitRaamUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BitRaamUnits::formatWithUnit(unit, amount, true));
...
void BitRaamGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitRaamGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitRaamGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitRaamGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitRaamGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BitRaamGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitRaamGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BitRaamGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BitRaamGUI::updateWindowTitle()
{
...
void BitRaamGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitRaamGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitRaamGUI::detectShutdown()
{
...
void BitRaamGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BitRaamGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BitRaamGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BitRaamGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BitRaamGUI::subscribeToCoreSignals()
{
...
void BitRaamGUI::unsubscribeFromCoreSignals()
{
...
bool BitRaamGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BitRaamUnit> units = BitRaamUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitRaamUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BitRaamUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BitRaamUnit u : BitRaamUnits::availableUnits()) {
        menu->addAction(BitRaamUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BitRaamUnit newUnits)
{
    setText(BitRaamUnits::longName(newUnits));
}
