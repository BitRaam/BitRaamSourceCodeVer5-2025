...
const QString BITCOIN_IPC_PREFIX("bitraam:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitraam: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click bitraam: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start bitraam: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling bitraam: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("bitraam://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'bitraam://' is not a valid URI. Use 'bitraam:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitraam: URI
    {
