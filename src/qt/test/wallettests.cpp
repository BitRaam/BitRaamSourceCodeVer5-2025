                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<BitRaamAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = BitRaamUnits::formatWithUnit(unit, expected_balance, false, BitRaamUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_bitraam-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_bitraam-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_bitraam-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    BitRaamAmountField* amountInput = receiveCoinsDialog.findChild<BitRaamAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_bitraam-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
