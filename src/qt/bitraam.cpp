...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BitRaamUnit>("BitRaamUnit");
#else
    qRegisterMetaType<BitRaamUnit>("BitRaamUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitraam_de.qm (shortcut "de" needs to be defined in bitraam.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. bitraam_de_DE.qm (shortcut "de_DE" needs to be defined in bitraam.qrc)
...
static const char* qt_argv = "bitraam-qt";

BitRaamApplication::BitRaamApplication()
...
void BitRaamApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BitRaamApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BitRaamGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BitRaamApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BitRaamApplication::createOptionsModel(bool resetSettings)
{
...
void BitRaamApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BitRaamGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BitRaamGUI::quitRequested, this, &BitRaamApplication::requestShutdown);
...
void BitRaamApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BitRaamApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BitRaamApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BitRaamApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BitRaamApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BitRaamApplication::handleRunawayException);
    connect(this, &BitRaamApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BitRaamApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BitRaamApplication::parameterSetup()
{
...
void BitRaamApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BitRaamApplication::requestInitialize()
{
...
void BitRaamApplication::requestShutdown()
{
...
void BitRaamApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // bitraam: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitRaamGUI::handlePaymentRequest);
            connect(window, &BitRaamGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BitRaamApplication::handleRunawayException(const QString &message)
{
...
void BitRaamApplication::handleNonFatalException(const QString& message)
{
...
WId BitRaamApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BitRaamApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitRaamGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(bitraam);
    Q_INIT_RESOURCE(bitraam_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see bitraam-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see bitraam-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse bitraam.conf, determine network, switch to network specific
    ...
    // bitraam: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
