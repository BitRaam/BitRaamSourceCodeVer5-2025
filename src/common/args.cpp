...
const char * const BITCOIN_CONF_FILENAME = "bitraam.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitraam-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\BitRaam
    //   new: C:\Users\Username\AppData\Local\BitRaam
    // macOS: ~/Library/Application Support/BitRaam
    // Unix-like: ~/.bitraam
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitRaam";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitRaam";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/BitRaam";
#else
    // Unix-like
    return pathRet / ".bitraam";
#endif
#endif
}
