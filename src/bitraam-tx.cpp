...
static void SetupBitRaamTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitraam-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitraam-tx tool is used for creating and modifying bitraam transactions.\n\n"
                "bitraam-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded bitraam transaction, or with \"-create [commands]\" to create a hex-encoded bitraam transaction.\n"
                "\n"
                "Usage: bitraam-tx [options] <hex-tx> [commands]\n"
                "or:    bitraam-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitraam transaction
