...
BOOST_AUTO_TEST_CASE(bnb_sffo_restriction)
{
    ...
    // Knapsack will only find a changeless solution on an exact match to the maharishi, SRD doesn’t look for changeless
...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 BRM in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 BRM in 1 coin
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19,8 BRM total --> 60 × 272 WU = 16320 WU
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 BRM total --> 10 × 272 WU = 2720 WU
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            return available_coins;
        });
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            // Expected Result: 4 + 3 + 2 + 1 = 10 BRM at 400 vB
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 10; ++j) {
                /* 10 × 1 BRM + 10 × 2 BRM = 30 BRM. 20 × 272 WU = 5440 WU */
...
BOOST_AUTO_TEST_CASE(check_max_selection_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 BRM and 1515x 0.033 BRM (~100.0 BRM total) unspent outputs
        // Then tries to spend 49.5 BRM
        // The 50.0 BRM output should be selected, because the transaction would otherwise be too large
        ...
        BOOST_CHECK(result);
        // Verify that the 50 BRM UTXO was selected, and result is below max_weight
...

    {
        ...
        // The actor starts with 400x 0.0625 BRM and 2000x 0.025 BRM (75.0 BRM total) unspent outputs
        // Then tries to spend 49.5 BRM
...

    {
        ...
        // The actor starts with 1515x 0.033 BRM (49.995 BRM total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 BRM
    }

    CAmount target{99900}; // 0.000999 BRM
