        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (coin_control.m_feerate && coin_selection_params.m_effective_feerate > *coin_control.m_feerate) {
        return util::Error{strprintf(_("Fee rate (%s) is lower than the minimum fee rate setting (%s)"), coin_control.m_feerate->ToString(FeeEstimateMode::SIT_VB), coin_selection_params.m_effective_feerate.ToString(FeeEstimateMode::SIT_VB))};
    }
    ...
    // 2. at least 1 sit greater than fees to spend it at m_discard_feerate
