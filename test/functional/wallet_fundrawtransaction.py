...
class RawTransactionsTest(BitRaamTestFramework):
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid bitraam address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sit/vB and feeRate BRM/kvB)")
        node = self.nodes[3]
        ...
        brm_kvb_to_sat_vb = 100000  # (1e5)
        result1 = node.fundrawtransaction(rawtx, fee_rate=str(2 * brm_kvb_to_sat_vb * self.min_relay_tx_fee))
        result2 = node.fundrawtransaction(rawtx, feeRate=2 * self.min_relay_tx_fee)
        result3 = node.fundrawtransaction(rawtx, fee_rate=10 * brm_kvb_to_sat_vb * self.min_relay_tx_fee)
        ...
        # With no arguments passed, expect fee of 141 sitashis.
        ...
        # Test fee_rate values that cannot be represented in sit/vB.
        for invalid_value in [0.0001, 0.00000001, 0.00099999, 31.99999999]:
            assert_raises_rpc_error(-3, "Invalid amount",
                node.fundrawtransaction, rawtx, fee_rate=invalid_value, add_inputs=True)

        self.log.info("Test min fee rate checks are bypassed with fundrawtxn, e.g. a fee_rate under 1 sit/vB is allowed")
        node.fundrawtransaction(rawtx, fee_rate=0.999, add_inputs=True)
        node.fundrawtransaction(rawtx, feeRate=0.00000999, add_inputs=True)

        self.log.info("- raises RPC error if both feeRate and fee_rate are passed")
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sit/vB) and feeRate (BRM/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (BRM/kvB)
        ...
        # Test subtract fee from outputs with fee_rate (sit/vB)
        brm_kvb_to_sat_vb = 100000  # (1e5)
        result = [self.nodes[3].fundrawtransaction(rawtx),  # uses self.min_relay_tx_fee (set by settxfee)
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[]),  # empty subtraction list
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0]),  # uses self.min_relay_tx_fee (set by settxfee)
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * brm_kvb_to_sat_vb * self.min_relay_tx_fee),
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * brm_kvb_to_sat_vb * self.min_relay_tx_fee, subtractFeeFromOutputs=[0]),]
        dec_tx = [self.nodes[3].decoderawtransaction(tx_['hex']) for tx_ in result]
        output = [d['vout'][1 - r['changepos']]['value'] for d, r in zip(dec_tx, result)]
        change = [d['vout'][r['changepos']]['value'] for d, r in zip(dec_tx, result)]
        ...
        # sitashis more, than outputs 2 and 3.
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 BRM outputs. The amount that we target for funding is in
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 BRM
        ...
        # Fund wallet with 2 outputs, 5 BRM each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 BRM each (20 BRM total)
        ...
        # Attempt to send 29 BRM from a wallet that only has 20 BRM. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 BRM payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 BRM.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 BRM)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 BRM.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 BRM (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 BRM/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # At a feerate of 1.85 sat/vb, the input will need a fee of 125.8 sits and the rest 77.7 sits
        # The entire tx fee should be 203.5 sits.
        ...
        # To test this does not happen, we subtract 202 sits from the input value. If working correctly, this should
        # fail with insufficient funds rather than bitraamd asserting.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 BRM target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 BRM) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 BRM covered (need to fund 0.9 BRM more).
